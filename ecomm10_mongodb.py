...
def log_event(user, pid, pname, action, extra=None):
    ...
    try:
        ...
        # Keep the app usable if MongoDB is temporarily unavailable.
        st.session_state.setdefault("_local_logs", []).append(event)
...
def save_order(user, name, address, items, geo=None):
    """Store a completed order in the MongoDB orders collection."""
    order = {
        "timestamp": datetime.datetime.utcnow(),
        "user": str(user) if user is not None else "guest",
        "customer_name": name,
...
def checkout():
    ...
    if st.button("Place Order"):
        ...
        if order_id:
            ...
            st.error("Order could not be saved to MongoDB. Please try again.")
